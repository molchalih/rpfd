...
| Work **accumulates in the working tree and lands as one commit when a unit is finished**, not as a commit per fix, per file, or per worker. A session that fixes six defects in one subsystem pushes one commit, not six. Push once a unit is complete rather than after each step | A log is a list of decisions, and a decision is not "an agent finished a task". Six commits for one session over one subsystem records how the work was scheduled, which is a fact about process; one commit records what changed, which is a fact about the archive format. History that reads as a queue of workers is noise |
